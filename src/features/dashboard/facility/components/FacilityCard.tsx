  ...
  return (
    <Card className="flex h-full flex-col overflow-hidden py-0 gap-0 rounded-2xl shadow-md hover:shadow-lg transition-all duration-300">
      ...
      <CardContent className="flex flex-1 flex-col p-5">
        <div>
          <h3 className="text-xl font-bold leading-tight line-clamp-2 min-h-14">{name}</h3>
          ...
          <div className="mt-2 flex items-start gap-2 text-muted-foreground min-h-6">
            ...
            <span className="line-clamp-1">{locationDetails}</span>
          </div>
        </div>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-xs">{maxCapacity} Personas</span>
          </div>

          <div className="flex items-center gap-2 justify-center">
            <Wallet className="h-5 w-5 text-muted-foreground" />

            <span className="text-xs font-bold text-primary">${hourlyRate.toFixed(2)}</span>
